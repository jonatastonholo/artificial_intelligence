...
def foodHeuristic(state, problem):
  ...
  if not foodGrid.asList():
    return 0
    solution = euclideanHeuristicFood(position, foodGrid.asList()[0], state)
    for food in foodGrid.asList():
        maze = euclideanHeuristicFood(position, food, state)
        if maze < solution:
            solution = maze
    return solution
  return 0

def euclideanHeuristicFood(position, problem):
  "The Euclidean distance heuristic for a PositionSearchProblem"
  xy1 = position
  xy2 = problem.goal
  return ( (xy1[0] - xy2[0]) ** 2 + (xy1[1] - xy2[1]) ** 2 ) ** 0.5
